...
from timm.models import resnet26d, resnet50d, register_model
...
class DistilledVisionTransformer(VisionTransformer):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            ...
            if cnt%4 == 0:
                feat = x[:,2:,]
                feat = feat.permute(0,2,1).unsqueeze(2).reshape([x[:,2:,].shape[0], self.embed_dim, self.seq_len, self.seq_len])
                feats.append(feat)
            cnt += 1 
...
def deit_base_distilled_patch16_384(
    ...
    model = DistilledVisionTransformer(
        img_size=img_size, patch_size=patch_size, seq_len=seq_len, embed_dim=embed_dim, depth=depth, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=True,
